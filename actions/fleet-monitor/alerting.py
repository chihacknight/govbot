"""Build the fleet's alert rules, contact point, and notification policy as data.
...
# The one label every fleet alert carries. Nothing routes on it — the stack is
# dedicated to the fleet monitor and the root policy catches everything (see
# build_policy()) — but it names the origin on every alert, and it is what a
# move to a shared stack would match on: adding a matcher then is one line,
# relabelling four provisioned rules is not.
...
def build_policy() -> dict:
    """The stack's entire notification policy tree, root and all.

    The stack is dedicated to the fleet monitor, so the fleet's contact point
    IS the root receiver: every alert the stack can produce is a fleet alert,
    and a stray one landing in Slack is a surprise worth seeing, not a routing
    case to file under a default receiver nobody reads. No matchers — the root
    matches everything by construction, and Grafana ignores matchers placed on
    it — and no child routes, which between them delete the tree-merging
    machinery an earlier design needed to join a stack it didn't own.
    ``provision-alerts`` still refuses to apply this over a tree it does not
    recognise (see ``alerts_provision.read_policy_tree``).

    ``group_by: [alertname]`` is the whole point of the policy. 47 jurisdictions
...
POLICY_HEADER = """\
# The stack's ENTIRE notification policy tree, generated by alerting.py.
#
# A real Grafana `policies:` document, on purpose: the stack is dedicated to the
# fleet monitor, so the fleet's contact point is the root receiver and every
# alert the stack produces lands in Slack and email — including a stray one,
# which should arrive as a visible surprise rather than vanish down a default
# receiver nobody reads. Applying this replaces the whole tree, which is why
# `main.py provision-alerts` first checks the tree it is replacing is its own
# (or a fresh stack's untouched default) and hard-stops on anything else.
"""
...
def render_documents() -> dict:
    ...
    return {
        ...
        POLICY_FILE: _document(
            POLICY_HEADER, {"apiVersion": 1, "policies": [{"orgId": 1, **build_policy()}]}
        ),
    }
