...
| Florida | fl | 🔄 In progress | 5 (stale, raw scraper wipes each run) | 2026-07-14 (re-diagnosed, not yet re-verified with fresh data) | **Correction 2026-07-14**: earlier notes said self-hosted still gets blocked (`RejectedResponse`) — that was based on a July 2 run. A 2026-07-13 self-hosted run got no bot-detection errors at all and ran the full 12h `timeout-minutes` ceiling before being killed mid-scrape (past bill 250+ of an estimated much larger session). Real problem: FL is just large and slow (many sequential per-bill fetches — detail page, House cross-reference, vote PDFs), and `scrape.sh` only commits after the full run completes, so hitting the timeout loses everything gathered. Fix applied: `timeout-minutes` raised to 1440 (24h) on the live workflow; a longer self-hosted run is in progress as of 2026-07-14. Real fix long-term is incremental commits during the run, not just a bigger timeout (PR [#5724](https://github.com/openstates/openstates-scrapers/pull/5724) + `fl-incremental-scraping-proposal.md` — still open) |
