> **Archived 2026-07-14.** This snapshot's methodology only checked bill counts, not whether
> that data was actually current — several entries marked `✅ OK` here (IL, NC, others) were
> already frozen at December 2025 data by the time this was written, and looked completely
> healthy anyway. See `actions/scrape/docs/error-tracking.md` for the live tracker, which
> adds a real freshness check, and `actions/scrape/docs/2026-07-14-recovery-summary.md` for
> what was found and fixed. Kept here for historical reference only — do not treat the
