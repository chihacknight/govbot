...
def process_config_file(config_file, script_dir, templates_dir, base_output_dir, test_states=None, all_states=False):
    ...
    # Filter locales based on test_states/all_states flags
    if not all_states:
        # Default: only process test states
        if test_states is None:
            test_states = ['al', 'ak', 'de', 'wy', 'sd']  # Default test states
        else:
            # Parse comma-separated string
            test_states = [s.strip() for s in test_states.split(',') if s.strip()]

        # Filter to only test states
        filtered_locales = []
        filtered_configs = []
        for locale, config_str in zip(all_locales, all_configs):
            if str(locale) in test_states:
        ...
        print(f"  🧪 Test mode: Processing {len(all_locales)} test states: {', '.join(test_states)}")
    else:
...
def main():
    ...
    for config_file in config_files:
        ...
        process_config_file(config_file, script_dir, templates_dir, base_output_dir, args.test_states, args.all_states)
