...
class GovbotFeedParser:
    ...
    def _item_to_bill(cls, item: ET.Element) -> Optional["Bill"]:
        ...
        Feed format (from govbot build):
          ...
          <link>https://example.com/repo/.../bills/HB1234/metadata.json</link>
          <description><![CDATA[
            id: HB1234
            log:
              action:
        ...
        title_raw   = text('title')        # "Tag1, Tag2 - repo - BILL TITLE"
        guid        = text('guid')         # repo path containing bill id
        pub_date    = text('pubDate')
        description = text('description')  # YAML-ish CDATA block

        # ── Extract bill identifier from the GUID path (most reliable) ──
        # guid looks like: il-legislation/country:us/state:il/sessions/104th/bills/HB2270/logs/...
        m = re.search(r'/bills/([HS][BCR]\d+)/', guid)
        if not m:
            # Fallback: try the title (last segment after " - ")
        ...
        # ── govbot <category> tags → subjects ───────────────────────────
        categories = [c.text.strip() for c in item.findall('category') if c.text]

        # ── Chamber from bill prefix ─────────────────────────────────────
        chamber = Chamber.HOUSE if bill_id.startswith('H') else Chamber.SENATE

        # ── Real bill title from <description> CDATA ─────────────────────
        # The feed <title> contains tags + repo + ALL-CAPS bill title.
        # The description CDATA has the readable title under bill: title:
        bill_title = ''
        abstract   = ''
        action_desc = ''
        action_date_str = ''
        if description:
            tm = re.search(r'^\s*title:\s*(.+)
...
def main():
    ...
    # When reading from the govbot feed, hearing dates are not available in
    # the OpenStates data, so we include ALL tagged bills (any subject) and
    # let the notification generator filter by subscription topic.
    now = datetime.now()
    future = now + timedelta(days=30)
    if args.feed:
        # Feed mode: include everything govbot tagged — subscriber topics
        # and the email digest do the filtering.
        actionable = [b for b in bills if b.subjects]
        print(f"📊 Total bills in feed: {len(bills)}")
        print(f"📊 Bills with tags: {len(actionable)}")
        # Show a sample of what we found
        for b in actionable[:5]:
            print(f"   • {b.bill_number}: {b.title[:60]} [{', '.join(b.subjects)}]")
        if len(actionable) > 5:
            print(f"   ... and {len(actionable) - 5} more")
    else:
        # Legacy/sample mode: filter for upcoming hearings or early readings
...
if __name__ == "__main__":
    main()
, description, re.M)
            acm = re.search(r'^\s*description:\s*(.+)
...
if __name__ == "__main__":
    main()
, description, re.M)
            adm = re.search(r'^\s*date:\s*(\d{4}-\d{2}-\d{2})', description, re.M)
            if tm:
                bill_title = tm.group(1).strip()
            if am:
                abstract = am.group(1).strip()
            if acm:
                action_desc = acm.group(1).strip()
            if adm:
                action_date_str = adm.group(1).strip()

        if not bill_title:
            # Last resort: ALL-CAPS segment from feed title
            parts = title_raw.split(' - ')
            bill_title = parts[-1].strip() if parts else title_raw

        # ── Hearing date: OpenStates doesn't carry future hearing dates ──
        # The action date is when the bill was last acted on, not a future
        # hearing. We keep it for recency filtering but don't treat it as a
        # scheduled hearing date.
        hearing_date: Optional[datetime] = None
        committee_name: Optional[str] = None

        # Check action description for committee assignment clues
        if action_desc:
            # "Assigned to Transportation Committee"
            # "Referred to Senate Transportation"
            cm = re.search(r'(?:assigned to|referred to|re-referred to)\s+(.+?)(?:\s+committee)?
...
,
        ...
        # ── Reading stage from action description ────────────────────────
        ad_lower = action_desc.lower()
        if 'third reading' in ad_lower:
            reading = BillReading.THIRD
        elif 'second reading' in ad_lower:
            reading = BillReading.SECOND
        else:
            reading = BillReading.FIRST

        # ── Build ILGA BillStatus URL from bill id (feed link = example.com) ─
        doc_type = 'HB' if chamber == Chamber.HOUSE else 'SB'
        bill_num = re.sub(r'^[HS][BCR]', '', bill_id)
        ilga_url = (
            f"https://www.ilga.gov/legislation/BillStatus.asp"
            f"?DocTypeID={doc_type}&DocNum={bill_num}&GAID=18&SessionID=114"
        )
        ...
        return Bill(
            ...
            chamber=chamber,
            title=bill_title,
            sponsor="Unknown",  # Not in govbot feed
