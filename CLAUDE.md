...
- **`.govbot/`** — the tool's **cache**, the `node_modules/` equivalent.
  Cloned datasets, content-addressed sync state, ledgers, an optional
  registry override. Fully regenerable; safe to `rm -rf` to start fresh.
  **Never edited by hand, never written to by `apply`.**
